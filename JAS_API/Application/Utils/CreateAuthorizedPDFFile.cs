...
{
    public class CreateAuthorizedPDFFile
    {
        public static byte[] CreateAuthorizedPDF(Valuation jewelry)
        {
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                Paragraph title = new Paragraph("GIAY UY QUYEN", titleFont)
                {
                    Alignment = Element.ALIGN_CENTER,
                    SpacingAfter = 20
                };
                ...
                document.Add(new Paragraph($"Email: {jewelry.Seller.Account.Email}", contentFont));
                document.Add(new Paragraph(" ", contentFont)); // Add a blank line
                ...
                // Add the "Customer Signature" line
                document.Add(new Paragraph("Customer Signature:", contentFont));

                document.Add(new Paragraph("tích", contentFont));
